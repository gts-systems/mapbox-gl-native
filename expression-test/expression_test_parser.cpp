...
optional<style::expression::type::Type> toExpressionType(const PropertySpec& spec) {
    ...
    const auto getType = [](const std::string& type) {
        ...
        assert(types.count(type));
        return types[type];
    };

    if (spec.type == "array") {
        type::Type itemType = spec.value.empty() ? type::Value : getType(spec.value);
        if (spec.length) {
    ...
    return spec.type.empty() ? nullopt : optional<style::expression::type::Type>{getType(spec.type)};
}
...
void parseCompiled(const JSValue& compiledValue, TestData& data) {
    ...
    const auto& result = toString(compiled["result"]);
...
std::vector<std::pair<std::string, std::string>> parseExpressionIgnores() {
    ...
    for (const auto& property : maybeIgnores.get<JSDocument>().GetObject()) {
        std::string ignore{ toString(property.name) };
        // Keep only expression-test ignores
        if (ignore.rfind("expression-tests", 0) != 0) {
            continue;
        }
        std::string reason{ toString(property.value) };
        ignores.emplace_back(std::make_pair(std::move(ignore), std::move(reason)));
    }
